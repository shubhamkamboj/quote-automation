...
def _is_too_similar(
    ...
    for previous in recent_quotes[-100:]:
        previous_normalized = _normalize_quote(previous)

        if not previous_normalized:
            continue

        # Catch exact and very-close rewrites.
...
""".strip()
    ...
    for attempt in range(1, 6):
        try:
            ...
            # Reserve the accepted quote in state so another post in the
            # same workflow cannot generate it again.
            recent_quotes.append(quote)
            used_hashes.add(quote_hash)

            state["recent_gemini_quotes"] = recent_quotes[-100:]
            ...
            last_error = exc
            print(
                f"Gemini quote attempt {attempt}/5 rejected: {exc}"
            )
...
def get_quote() -> tuple[str, str, int | None]:
    ...
    # Post #1 may use priority.txt.
    # Posts #2-#4 explicitly use Gemini.
    if not force_gemini:
